...
print(f"{bcolors.OKGREEN} Status Dev Tools: OK{bcolors.ENDC}") if r.status_code == 200 else print(f"{bcolors.FAIL} Status Dev Tools: OK{bcolors.ENDC}")

r = s.post(rundeck_url + "/api/27/job/9a190daf-3a6c-4d9f-8f28-e4d4a771ffbc/run",
           data=jobdata,
           headers=requestheaders)

print(f"{bcolors.OKGREEN} Status Non-prod: OK{bcolors.ENDC}") if r.status_code == 200 else print(f"{bcolors.FAIL} Status Non-prod: NOK{bcolors.ENDC}")

r = s.post(rundeck_url + "/api/27/job/cde17503-06eb-4b09-b5fc-8388e0fa1902/run",
           data=jobdata,
           headers=requestheaders)

print(f"{bcolors.OKGREEN} Status Prod: OK{bcolors.ENDC}") if r.status_code == 200 else print(f"{bcolors.FAIL} Status Prod: NOK{bcolors.ENDC}")
